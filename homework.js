...
1. JSON 數據處理方式: Axios： 自動處理。它會自動將回應的數據轉換成 JSON 格式，fetch 需要手動調用 .json() 方法。
 
2. 錯誤處理機制（HTTP 狀態碼）: Axios： 自動處理。它會根據 HTTP 狀態碼拋出錯誤，fetch 需要手動檢查狀態碼。
...
if (require.main === module) {
  ...
  async function runTests() {
    ...
    if (API_PATH) {
      ...
      /* try {
        const result = await addToCartWithAxios("CIoPCmMTjRaxfqnJBe0G", 3);
